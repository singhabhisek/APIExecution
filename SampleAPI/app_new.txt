...
log_filename = os.path.join(log_directory, "service_execution2.log")
...
def process_item(item, output, app_name, request_tracker={}, service_name=None, postman_data=None):
    for sub_item in item:
        ...
        if 'item' in sub_item:
            ...
            sample_request_location = f"static/{app_name}/{final_service_name}/{final_operation_name}.txt"
            ...
            if body and 'raw' in body:
                raw_body = body['raw']
                file_path = os.path.join(TRANSFORMED_FOLDER, sample_request_location)
...
# Function to transform the Postman collection JSON
def transform_postman_collection(input_file_path, output_file_path, app_name):
    # # Load the input Postman collection JSON
    # with open(input_file_path, 'r') as file:
    #     postman_data = json.load(file)  # Read the Postman collection from the specified file
    # Read the content of the FileStorage object
    postman_data = json.loads(input_file_path.read())  # Read the content directly

    # Prepare the output structure
    output = {"entries": []}  # Initialize the output data structure
    request_tracker = {}  # Track multiple operation names for uniqueness

    # Start processing the items in the Postman collection
    process_item(postman_data.get('item', []), output, app_name, request_tracker=request_tracker, postman_data=postman_data)

    # Write the transformed data to an output file
    with open(output_file_path, 'w') as outfile:
        json.dump(output, outfile, indent=4)  # Save the transformed output to the specified file

    print("Transformation completed successfully!")  # Print a completion message
    log_action(action="Postman Collection Transformed", url=output_file_path)
                            ...
                            if method == 'POST':
                                ...
                                if entry['sampleRequestLocation'] != '':
                                    ...
                                    try:
                                        ...
                                        # Step 3: Attempt custom logic to handle XML with namespaces
                                        try:
                                            def parse_xml_with_dynamic_namespaces(xml_content):
                                                """
                                                This function handles namespaces dynamically in XML content.
                                                """
                                                xml_root = ET.fromstring(xml_content)

                                                # Extract namespaces dynamically (if any)
                                                namespaces = dict([
                                                    node for _, node in ET.iterparse(
                                                        StringIO(xml_content), events=['start-ns']
                                                    )
                                                ])
                                                return xml_root, namespaces

                                            # Attempt to parse the XML with custom namespace handling
                                            xml_root, namespaces = parse_xml_with_dynamic_namespaces(
                                                sample_request_content)

                                            # Convert the parsed XML back to a string
                                            xml_string = ET.tostring(xml_root, encoding='unicode')

                                            # Populate raw_request with the parsed XML data
                                            raw_request['data'] = xml_string
                                            raw_request['xml'] = xml_string
                                            raw_request['headers']['Content-Type'] = 'application/xml'

                                        except Exception as e2:
                                            print(f"Custom XML parsing with namespaces also failed: {e2}")

                                            # Step 4: Fallback to JSON parsing if XML parsing failed
                                            try:
